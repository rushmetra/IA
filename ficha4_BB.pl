...
ciclo(G,A,P):- adjacente(B,A,_,_,G), caminho(G,A,B,P1), length(P1,L), L>2, append(P1,[A],P).

%--------------------------------- 
%alinea 4)

caminhoK(G,A,B,P, Km, Es):- caminho1K(G,A,[B],P,K,Es).
caminho1K(_,A,[A|P1],[A|P1],0,[]).
caminho1K(G,A,[Y|P1],P,K1, [E|ES]):-
	adjacente(X,Y,E,Ki, G), 
	nao(membro(X, [Y|P1])),
caminho1K(G,A,[X,Y|P1],P,K,Es), K1 is K + Ki.


%--------------------------------- 
%alinea 5)

cicloK(G,A,P, Km, [E|Ep1]):- 
	adjacente(B,A,E,K,G), 
